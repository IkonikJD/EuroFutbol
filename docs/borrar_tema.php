...
$titulo = "EuroFutbol - Eliminar Tema";
...
if (isset($_POST['enviar'])) {
    borrarTema($varsesion, $_POST, $id, $nombre);
} else {
    ?>
    ...
    if (!isset($varsesion) || empty($varsesion)) {
        ...
        <div class="container">
            <div class="jumbotron" id="forobloqueado"> <a class="btn btn-primary botonvolver btn-success" href="javascript: history.go(-1)" role="button">Volver</a>
                <h1 class="text-center">Borrar Tema</h1>
                <p class="text-center" id="avisoborrado">Antes de eliminar un tema, debemos comprobar que seas el
                    autor del tema, por favor, introduce tu contraseña</p>
                <form role="form" id="form1" method="POST" action="borrar_tema.php<?php echo "?id=$id&nombre=$nombre"; ?>"> 
                    <div class="form-group text-center">
                        ...
                        <button type="submit" id="enviar" name="enviar" class="btn btn-default btn-success">Eliminar Tema</button>
                    </div>
                </form>
            </div>
        </div>
